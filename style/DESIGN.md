...
| On-primary | `#f8fbff` | `#0c1222` | Ink on primary-filled and kept buttons (Kept, Create item, submits). Dark mode inverts it because primary itself is lightened for links |
...
- **Item Kept Toggle**: Two-state toggle button on item lists indicating collection membership.
  - **Kept (`aria-pressed="true"`)**: Filled with `success` token (`--success`) and `on-primary` text (`var(--on-primary)`).
  - **Keep (`aria-pressed="false"`)**: Secondary ghost style with no fill, `outline_strong` border, and `primary` text.
