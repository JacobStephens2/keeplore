  ...
  return [
    ...
    'endpoints' => [
      [
        ...
        'method' => 'POST',
        'path' => '/artifacts.php',
        'summary' => 'List, search, and filter items in the key\'s collection.',
        'body_fields' => [
          'page', 'per_page', 'cursor', 'query', 'tag', 'kept', 'physical', 'digital',
          'secondary_collection', 'type_id', 'fields', 'include',
        ],
        'fields_values' => ['basic', 'collection'],
        'include_values' => ['uses_summary'],
        'example_body' => '{"kept":true,"type_id":[26,39],"physical":true,"fields":"collection","include":["uses_summary"],"per_page":200}',
        'get_lists_first_page' => true,
        'notes' => [
          'Agent keys always read their own user; a userid in the body is ignored.',
          'Default page size is 50; maximum is 200. has_more says whether another page exists. Send cursor (null, then next_cursor) to page by id instead of by title.',
      ...
      [
        ...
        'notes' => [
          ...
          'tags are the owner\'s free-form labels (portable, beach-safe, two-player, party). They complement player-count and time columns; they do not replace them. Most items have no tags yet, so expect an empty list.',
        ],
      ],
      [
        'id' => 'list-uses',
        ...
        'query_fields' => ['artifact_id', 'player_id'],
        'example_query' => '?player_id=45',
        'notes' => [
          'Omit artifact_id to list every use. A use is an actual interaction, not an item proposal.',
          'player_id keeps only uses that player took part in; ids come from GET /players.php.',
          'Each use carries players (deduplicated player ids) and participants (id, FirstName, LastName).',
        ],
      ],
      [
        'id' => 'list-players',
        'method' => 'GET',
        'path' => '/players.php',
        'summary' => 'List the people recorded in the key\'s household.',
        'query_fields' => [],
        'notes' => [
          'Each player has id, name, FirstName, LastName, birth_year, and represents_user_id (set when the player is a Keeplore user).',
        ],
      ],
      [
        'id' => 'list-proposals',
